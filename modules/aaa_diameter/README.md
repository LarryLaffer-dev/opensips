...
be sent using the [dm_send_request()](#dm_send_requestapp_id-cmd_code-avps_json-rpl_avps_pv) function.
...
to process [E_DM_REQUEST](#e_dm_request) events in
...
are prepared, script writers should use the [dm_send_answer()](#dm_send_answeravps_json-is_error)
...
Time, in milliseconds, after which a [dm_send_request()](#dm_send_requestapp_id-cmd_code-avps_json-rpl_avps_pv)
...
```opensips title="Setting the max_json_log_size parameter"
...
Similar to [dm_send_request()](#dm_send_requestapp_id-cmd_code-avps_json-rpl_avps_pv) but performs an asynchronous Diameter request.


Uses the same parameters and return codes as
[dm_send_request()](#dm_send_requestapp_id-cmd_code-avps_json-rpl_avps_pv).
...
since that is the only way to gain access to the [dm_send_answer()](#dm_send_answeravps_json-is_error)
