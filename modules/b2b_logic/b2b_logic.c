...
/* upper bound for the non-blocking acquire below. Sized well above the longest
 * legitimate hold time of a tuple lock - a WRITE_THROUGH b2bl_db_update - so
 * that it only ever trips on an actual lock-order inversion */
#define B2BL_AUX_TRYLOCK_MAX_MS 1000

/* Acquire a tuple lock for reading on a path that may be re-entered from
 * b2b_entities, without risking an ABBA deadlock.
 *
 * b2b_logic holds the tuple lock while calling into b2b_entities, which locks
 * the entities hash. b2b_entities then runs TM callbacks - rtp_relay, for one -
 * while holding that hash lock, and those callbacks read the b2b_logic key back
 * through here. Two processes handling the two directions of the same call can
 * end up each holding the lock the other is waiting for. Backing off instead of
 * blocking lets the peer finish and release.
 *
 * Returns 1 with the lock held (freshly taken, or already owned by this process
 * on a re-entrant call), 0 if it could not be taken within the budget. Like
 * B2BL_LOCK_GET_AUX, a fresh acquire does not set locked_by, so
 * B2BL_LOCK_RELEASE_AUX is the matching release. */
static int b2bl_trylock_aux(unsigned int hash_index)
{
	int i;
		...
		int try_lock)
{
	...
	*locked = 1;
	if (try_lock) {
		if (!b2bl_trylock_aux(ctx->hash_index)) {
			LM_WARN("tuple lock [%u] busy for %dms, backing off to avoid a "
...
static str *b2bl_get_key(void)
{
	...
	int locked = 0;
	/* reached from TM callbacks that run under the b2b_entities hash lock, so
	 * this one must not block on the tuple lock */
