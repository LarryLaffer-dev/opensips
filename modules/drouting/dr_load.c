                  ...
                  int persistent_state, str *rules_tables, int rules_tables_no,MD5_CTX* hash_ctx)
{
	...
	for (j = 0; j < rules_tables_no; j++) {
		...
		if (DB_CAPABILITY(*dr_dbf, DB_CAP_FETCH)) {
			...
			no_rows = estimate_available_rows( 4+32+32+128+32+64+128+4+1+1, 11/*cols*/);
			if (no_rows==0) no_rows = 10;
			if(dr_dbf->fetch_result(db_hdl, &res, no_rows)<0) {
		...
		do {
			for(i=0; i < RES_ROW_N(res); i++) {
				...
				/* DISABLED column - skip the rule entirely when flagged
				 * disabled, so dr_reload honours it. A NULL or unexpected
				 * type is treated as enabled to stay backwards compatible. */
				if ( VAL_TYPE(ROW_VALUES(row)+10)==DB_INT &&
						VAL_INT(ROW_VALUES(row)+10)!=0 ) {
					LM_DBG("skipping disabled rule id %d\n",
