...
/* highest backreference (\0..\5) usable in a NAPTR replacement expression */
#define LOST_RE_MAX_MATCH 6

/*
 * Expand a NAPTR replacement expression, substituting \0..\5 with the
 * corresponding capture from a previous regexec().
 *
 * @result: in, .s points to the output buffer and .len is its capacity;
 *          out, .len is the number of bytes written
 *
 * Returns 1 on success, negative on error.
 *
 * OpenSIPS has no reg_replace() equivalent -- its subst_str() only accepts
 * "s/pattern/replacement/flags" syntax, whose '/' delimiter collides with the
 * URLs that LoST/HELD NAPTR records expand to. So this mirrors Kamailio's
 * core reg_replace()/replace() pair, which is what the upstream module used.
 */
static int lost_replace(
		regmatch_t *pmatch, char *string, char *replacement, str *result)
{
	...
	for(i = 0; i < len; i++) {
		if(replacement[i] == '\\') {
			...
			if(isdigit((unsigned char)replacement[i + 1])) {
				...
				if(digit >= LOST_RE_MAX_MATCH || pmatch[digit].rm_so == -1)
					return -2;
			...
			/* escaped literal: emit the character that follows */
			i++;
		}
...
static int lost_reg_replace(
		char *pattern, char *replacement, char *string, str *result)
{
	regmatch_t pmatch[LOST_RE_MAX_MATCH];
	regex_t preg;
	...
	if(preg.re_nsub > LOST_RE_MAX_MATCH) {
		regfree(&preg);
		return -1;
	}

	if(regexec(&preg, string, LOST_RE_MAX_MATCH, pmatch, 0)) {
		regfree(&preg);
		return -1;
	}
	regfree(&preg);

	return lost_replace(&pmatch[0], string, replacement, result);
}
...
int lost_naptr_lookup(str hostname, str *service, str *result)
{
	...
	/* OpenSIPS' get_record() takes no flags argument and already returns
	 * records of the requested type only */
	head = get_record(hostname.s, T_NAPTR);
	...
	for(l = head; l; l = l->next) {
		...
		/* replace hostname */
		if(lost_reg_replace(pattern.s, replacement.s, &(hostname.s[0]), result)
				< 0) {
