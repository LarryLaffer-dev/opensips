...
/*
 * lost_part_content_id(part, cid)
 * finds the Content-ID header of a MIME body part and returns its value,
 * angle brackets included, so that it can be compared with a "cid:" URL
 * as referenced from a Geolocation header (RFC 4483)
 */
static int lost_part_content_id(const struct body_part *part, str *cid)
{
	static const str hname = str_init("Content-ID");
	char *p, *end, *eol, *colon;
	str line, name;

	p = part->headers.s;
	end = p + part->headers.len;

	while(p < end) {
		eol = q_memchr(p, '\n', end - p);
		line.s = p;
		line.len = (eol ? eol : end) - p;

		colon = q_memchr(line.s, ':', line.len);
		if(colon) {
			name.s = line.s;
			name.len = colon - line.s;
			trim(&name);

			if(str_casematch(&name, &hname)) {
				cid->s = colon + 1;
				cid->len = line.s + line.len - cid->s;
				trim(cid);
				return 0;
			}
		}
...
/*
 * lost_get_pidf_by_cid(msg, cid, lgth)
 * returns the body part referenced by a Geolocation "cid:" URL, which is how
 * a UE conveys its PIDF-LO by value (RFC 6442, TS 24.229)
 *
 * The returned pointer is into the message buffer and must not be freed.
 * Replaces Kamailio's get_body_part_by_filter(), which OpenSIPS lacks.
 */
char *lost_get_pidf_by_cid(struct sip_msg *msg, char *cid, int *lgth)
{
	...
	*lgth = 0;
	...
	if(parse_sip_body(msg) < 0 || msg->body == NULL) {
		LM_WARN("failed to parse the SIP body\n");
		return NULL;
	}
	...
	for(part = &msg->body->first; part; part = part->next) {
		...
		if(lost_part_content_id(part, &part_cid) != 0)
			continue;

		LM_DBG("body part Content-ID: [%.*s]\n", part_cid.len, part_cid.s);

		if(str_casematch(&part_cid, &scid)) {
			*lgth = part->body.len;
			return part->body.s;
		}
	}

	LM_WARN("no body part with Content-ID [%s] found\n", cid);
...
{
	struct to_body *p_pai;
	char *res = NULL;

	*lgth = 0;

	if((parse_pai_header(msg) == 0) && (msg->pai) && (msg->pai->parsed)) {
		/* OpenSIPS parses P-A-I as a multi name-addr header, so get_pai()
		 * is already the first identity of a ->next linked list (Kamailio
		 * instead wraps them in an id[]/num_ids array) */
		p_pai = get_pai(msg);
