...
void ipsec_ctx_release(struct ipsec_ctx *ctx)
{
	...
	if (!free) {
		LM_DBG("IPSec ctx %p still referenced (ref=%d)\n", ctx, ctx->ref);
		return;
	}

	/* last reference: unlink from the user and from the TMP timer list
	 * before freeing, so neither is left pointing at dead memory */
	if (ctx->user) {
		ipsec_ctx_release_user(ctx);
...
void ipsec_ctx_release_user(struct ipsec_ctx *ctx)
{
	...
	/* the ctx may already have been unlinked by a concurrent teardown */
	if (list_is_valid(&ctx->list))
		list_del(&ctx->list);

	/* the user only exists to hold SAs, so drop it once the last one goes */
	release = list_empty(&user->sas);

	LM_DBG("released ctx %p from user %.*s, %s\n", ctx,
			user->impi.len, user->impi.s,
			release ? "no SAs left" : "SAs remaining");
...
void ipsec_ctx_timer(unsigned int ticks, void* param)
{
	...
	list_for_each_safe(it, safe, &new) {
		...
		if (VALID_IPSEC_STATE(tmp->ctx->state)) {
			...
			/* already torn down elsewhere; just drop the timer entry */
			LM_DBG("IPSec ctx %p already released\n", tmp->ctx);
			list_del(&tmp->list);
			shm_free(tmp);
		}
	}
