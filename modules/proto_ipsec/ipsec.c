...
void ipsec_ctx_release(struct ipsec_ctx *ctx)
{
	...
	LM_ERR("Releasing IPSec ctx %p (state %d), ref %d\n", ctx, ctx?ctx->state:0, ctx?ctx->ref:0);
	...
	if (!VALID_IPSEC_STATE(ctx->state)) {
		LM_ERR("ctx %p is not in a valid state %d\n", ctx, ctx->state);
		return;
	}
	...
	if (free) {
		...
		LM_ERR("IPSec ctx %p not released, ref=%d\n", ctx, ctx->ref);
	}
...
void ipsec_ctx_timer(unsigned int ticks, void* param)
{
	...
	list_for_each_safe(it, safe, ipsec_tmp_contexts) {
		...
		LM_ERR("Context %p (state %d) expire at %u, current ticks %u\n",
				tmp->ctx, tmp->ctx->state, (unsigned int)tmp->expire, ticks);
		if (ticks < tmp->expire)
			break; /* finished */
		LM_ERR("IPSec ctx %p removing\n", tmp->ctx);
		prev = it;
	}
	if (!prev) {
	...
	list_for_each_safe(it, safe, &new) {
		...
		if (VALID_IPSEC_STATE(tmp->ctx->state)) {
			...
			LM_ERR("Released lock for context %p (state %d), free=%d\n", ctx, ctx->state, free);
			shm_free(tmp);
			if (free)
				ipsec_ctx_free(ctx);
			LM_ERR("IPSec ctx %p deleted\n", ctx);
		} else {
	...
	LM_ERR("Finished removing expired contexts\n");
}
