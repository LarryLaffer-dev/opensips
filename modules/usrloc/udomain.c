...
int preload_udomain(db_con_t* _c, udomain_t* _d)
{
	...
	do {
		...
		for(i = 0; i < RES_ROW_N(res); i++) {
			 ...
			 * the restored K/V store.  Raised only for the contact that
			 * survives the cid_regen path, and kept distinct from
			 * UL_CONTACT_INSERT so modules acting on live registrations
			 * (e.g. pua_reginfo) are not triggered by a startup preload. */
